...
namespace neon
{
    // "Chrome & neon" design (hardware-render PDR): a polished metallic-black
    // faceplate floating on a black void, neon rim light, neon around the
    // screen, dark inset wells for the control bank and display.
    //
    // DEXTRO DELAY palette: the skill's pink+cyan scheme is re-tuned to
    // PURPLE + a DARKER HUE OF BLUE while keeping every drawing helper and
    // layout untouched — the token names below stay the same (so the
    // LookAndFeel is still drop-in), only their colour VALUES change. The
    // `neonPurple` / `neonBlue` aliases are provided for readable new code.
    ...
    inline juce::Colour accent()    { return neonPurple; }
    inline juce::Colour accentDim() { return neonMagenta.withAlpha (0.55f); }

    // Control accents share the purple->blue neon language; the entries
    // alternate hue so a bank of controls reads with variety.
    inline const juce::Colour voiceColours[4] { neonPurple, neonBlue, neonMagenta, neonViolet };
    ...
    // than the inset panels) with fine streaks and a subtle top sheen. The
    // knobs/buttons keep their own chrome — this is only the background plate.
